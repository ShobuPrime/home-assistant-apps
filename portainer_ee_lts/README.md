...
**Issue**: Starting any container from the Portainer UI failed with
`Failed starting container: starting container with non-empty request body was
deprecated since API v1.22 and removed in v1.24`. Because Docker performs that
check before it looks at the container, the error also *hid* the real reason a
container would not start.

**Fix**: This app routes Portainer's Docker API through a small nginx shim that
drops the request body on the container lifecycle calls where Docker does not
allow one, and passes everything else through unchanged. Existing installs need
no migration — see [DOCS.md](DOCS.md#docker-socket-shim). If you are still
seeing this error, rebuild the app.
...
Currently running Portainer 2.45.0 LTS
