...
### Critical: Docker Socket Shim

**Portainer never talks to dockerd directly in this app.** It talks to nginx,
which forwards to the real socket:

```
portainer --host unix:///var/run/docker.sock
  -> /var/run/docker.sock          (symlink, created by cont-init)
  -> /run/docker-shim/docker.sock  (nginx, rootfs/etc/nginx/docker-shim.conf)
  -> /run/docker.sock              (mounted by the Supervisor)
```

**Why it exists.** Docker removed HostConfig-in-the-body from
...
transfer encoding it received, and requests arriving through Home Assistant's
streaming ingress are chunked — so every container start failed with
`starting container with non-empty request body was deprecated since API v1.22
and removed in v1.24`. The check runs in dockerd's HTTP handler before any
container lookup, which is why the message names no container and why it
*masks* the real start error. Portainer EE is a closed binary and HAOS's
dockerd is not configurable, so the app is the only layer that can fix it.

**Rules when touching this:**

- **Only strip bodies where a body is illegal.** The location regex covers
  `start|stop|restart|kill|pause|unpause|wait` under `/containers/{id}/`.
  `/exec/{id}/start` takes a real JSON body (`Detach`, `Tty`) and `…/attach`
  needs the upgrade tunnel — adding either silently breaks the console.
- **The shim runs its own config via `nginx -c`, not Alpine's nginx.conf.**
  Alpine's stock config already defines `$connection_upgrade` (a second `map`
  for it is a fatal duplicate) and caps request bodies at 1m, which would 413
  image builds and large container creates.
- **Workers run as root.** The socket is `root:docker` 0660; the `nginx` user
  gets EACCES and every request 502s.
- **The socket lives in a 0700 directory.** nginx chmods unix listen sockets
  to 0666 unconditionally, so the directory is the only thing keeping the
  proxied Docker API root-only inside the container.
- **Do not "fix" the migration by pointing `--host` at a new socket.**
  Portainer stores its environment address in BoltDB and `--host` is ignored
  once any environment exists ("instance already has defined environments"), so
  that would fix nothing for existing installs and force everyone to re-create
  their environment — orphaning their stacks. The redirect of
  `/var/run/docker.sock` is what makes the fix reach installs that already
  exist. That is also why `--host unix:///var/run/docker.sock` must stay
  exactly as it is.
- **The smoke test enforces all of this** (`.github/scripts/smoke-test.sh`,
  `portainer_ee_lts|portainer_ee_sts` case): it reproduces the 400 on the raw
  socket, requires 304 through the shim on both the versioned and bare API
  paths, starts and stops a real scratch container, runs a hijacked exec, and
  pushes a 2 MB body through.
