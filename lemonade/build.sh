...
echo -e "${BLUE}=== Lemonade App Builder ===${NC}"
...
LEMONADE_VERSION=$(grep "LEMONADE_VERSION:" build.yaml | cut -d':' -f2 | tr -d ' ')
echo -e "Lemonade version: ${YELLOW}${LEMONADE_VERSION}${NC}"
...
docker build \
    ...
    --build-arg BUILD_DESCRIPTION="Lemonade for Home Assistant" \
...
if [ $? -eq 0 ]; then
    ...
    echo -e "  ${YELLOW}docker run --rm -it -p 13305:13305 ${FULL_IMAGE}${NC}"
else
    echo ""
