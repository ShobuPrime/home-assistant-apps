...
Name of your general-purpose vault, created on first startup. Default: `default`. A second vault, `ha_vault` (default `home_assistant`), is created alongside it and is dedicated to Home Assistant — keeping voice-assistant traffic from crowding out your own notes in the relevance ranking. Both are created public (no API key required). Set either to empty to skip creating it.
...
2. Optionally rename `default_vault` (`default`) and `ha_vault` (`home_assistant`)
3. Start the app
...
Currently running MuninnDB 0.11.0
