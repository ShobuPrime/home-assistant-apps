...
### Vaults created on first startup

Two vaults are provisioned automatically so memory contexts stay separated:

| Option | Default | Purpose |
|--------|---------|---------|
| `default_vault` | `default` | Your general-purpose vault — the one your AI tools use over MCP. MuninnDB already ships an open vault of this name, so the default is effectively a no-op. |
| `ha_vault` | `home_assistant` | Dedicated to Home Assistant, e.g. voice-assistant conversations. |

**Why two.** MuninnDB ranks memories by how recently and how often they are
recalled. A voice assistant writes far more often than a person does, so
pointing it at your general vault would let routine "turn on the kitchen light"
traffic outrank the notes you actually care about. Separate vaults keep each
one's relevance ranking meaningful.

Both are created **public** by default (`default_vault_public` /
`ha_vault_public`), meaning no API key is required — this is what lets other
add-ons on your network read and write them without credential plumbing. Turn
either off to require an API key for that vault.

> Note that vaults you create yourself through the Web UI or REST API start
> **locked**; only the built-in `default` vault and the ones this app
> provisions are opened for you.

Set either option to empty to skip creating that vault. If both are set to the
same name only one vault is created, and the app logs a warning — sharing a
single vault defeats the separation described above.
...
The bundled embedder is `bge-small-en-v1.5` — English-only, and fixed: it
cannot load a different model. To use another one, point `ollama_url` at any
Ollama-compatible server that hosts it. MuninnDB then just sends text and
receives vectors, so the model's format (GGUF, ONNX, anything) is that server's
concern, not MuninnDB's.

**The model name is part of the URL.** Omitting it will not work:

```
ollama://<host>:<port>/<model-name>
```

#### Pairing with the Lemonade app

If you run the Lemonade app for a conversation agent, it can serve the
embedding model too — keeping one model server, and letting you use the
embedding counterpart of your chat model:

1. In **Lemonade**, set `embedding_model_name` to e.g. `LFM2.5-Embedding`.
   It downloads the model, serves it, and pins it in memory.
2. In **MuninnDB**, set `ollama_url` to
   `ollama://<lemonade-hostname>:13305/LFM2.5-Embedding`
   (the hostname is on Lemonade's Info tab).

Lemonade handles downloading, caching and updating the model — MuninnDB never
fetches models itself. Lemonade caps loaded models per *type*, so an embedding
model and a chat model stay resident at the same time without evicting each
other.

Sizes for the defaults: the LFM2.5 chat model is ~150 MB and the
LFM2.5-Embedding-350M is ~360 MB, so both together sit under 600 MB.
...
vault. Dimensions differ too — the bundled model produces 384, LFM2.5-Embedding
produces 1024.
...
2. Optionally rename the two vaults — `default_vault` (`default`) and `ha_vault` (`home_assistant`)
3. Optionally configure embedding providers for semantic search
4. Start the app — it will set your password and create both vaults
