...
# Supervisor 2026.07.4 renamed app containers from addon_<slug> to app_<slug>
# (supervisor/docker/app.py: `return f"app_{slug}"`, with a migration that renames
# existing addon_* containers). Try the new name first, keep the old one so this
# still resolves on a Supervisor that predates the rename.
HAOS_CID="app_$(echo "${CONTAINER_HOSTNAME}" | sed 's/-/_/g')"
HAOS_CID_LEGACY="addon_$(echo "${CONTAINER_HOSTNAME}" | sed 's/-/_/g')"

for CONTAINER_ID in ${PROC_CID} ${HAOS_CID} ${HAOS_CID_LEGACY} ${CONTAINER_HOSTNAME}; do
