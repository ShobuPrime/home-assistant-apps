...
APP_INFO = {
    ...
    "data": {
        ...
        # Published container->host port map. Apps that build a browser-facing
        # allowlist must use the port the BROWSER sees, which a user may remap
        # (Lemonade's ports_description invites 13305 -> 11434 for Ollama
        # auto-detection). Deriving from the app's own bind port instead would
        # silently produce a list that never matches.
        "network": {"13305/tcp": 13305},
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        routes = {
            ...
            # Host addresses. Lemonade derives its browser-Origin allowlist
            # from these, because lemond rejects unknown Origins with 403 and
            # its built-in defaults cover localhost only. Both an IPv4 and an
            # IPv6 address are advertised so the bracketing of v6 literals in
            # an origin is actually exercised.
            "/network/info": {
                "result": "ok",
                "data": {
                    "interfaces": [
            ...
            # Home Assistant's own URLs — the origin the browser sends when the
            # panel is opened through ingress, and therefore the one that makes
            # the ingress panel work rather than 403.
            "/core/api/config": {
