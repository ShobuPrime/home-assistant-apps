...
#   4. Signals must be RECEIVABLE, not just sendable. AppArmor mediates
#      signal delivery as well as sending, so a profile that grants only
#      `signal (send) ...` — or omits signal rules entirely — stops s6's
#      SIGTERM from ever reaching the app: the graceful shutdown is skipped
#      and the container is SIGKILLed after the grace period (exit 137).
#      Measured on one identical image (lemonade), varying only this rule:
#        no signal rule           -> 13.2s, exit 137, no cleanup logged
#        signal (send) set=(...)  -> 12.7s, exit 137, no cleanup logged
#        signal (send,receive),   ->  6.9s, exit 0, full s6 shutdown
#      Accepted forms: a bare `signal,` or any signal rule granting `receive`.
#
...
for profile_file in */apparmor.txt; do
    ...
    # Rule 4: signals must be receivable, not just sendable
    if ! grep -qE '^[[:space:]]*signal[[:space:](,]' "${profile_file}"; then
        err "${profile_file}: no 'signal' rule — AppArmor then denies signal delivery, so s6's SIGTERM never reaches the app and it is SIGKILLed on stop (exit 137). Add 'signal (send,receive),'"
    elif ! grep -qE '^[[:space:]]*signal[[:space:]]*,[[:space:]]*$' "${profile_file}" \
        && ! grep -qE '^[[:space:]]*signal[^#]*receive' "${profile_file}"; then
        err "${profile_file}: signal rule does not grant 'receive' — s6's SIGTERM never reaches the app, so the graceful shutdown is skipped and the container is SIGKILLed (exit 137). Use 'signal (send,receive),'"
    fi
done
