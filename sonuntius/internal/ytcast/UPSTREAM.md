...
Verify with: `cd ~/src/yt-cast-receiver-upstream && git rev-parse HEAD`

## Update workflow

When updating to a newer upstream commit:

1. Fetch + checkout the new commit in `~/src/yt-cast-receiver-upstream`.
...
| n/a | `cmd/yt-cast/main.go` | Go-only sonuntius binary. Reads app options via `internal/config`, derives a stable receiver UUID under `/data/sonuntius/`, dials the ma-bridge IPC socket with backoff, builds the receiver with retry-with-backoff on `Start` failure so the app container stays healthy even when its dependencies aren't. Logs the pinned upstream commit short SHA on startup |
