...
The app slug must appear in `.github/scripts/update-base-image.sh` in the `APP_DIRS` variable. That is the **only** registration step.

The PR body's "Affected Apps" list is derived from the actual diff (`.github/workflows/update-base-image.yml`, the `update` step) — do **not** add the app to a list there. It used to be a hand-maintained literal and it drifted: it named 6 apps under the heading "All apps using the hassio-addons base image" while the updater changed 11, so PR #191 silently omitted `aegis_ha`, `hay_cm5_fan`, `lemonade`, `muninndb` and `sonuntius`. Fixed in #196.
...
The app slug must be in that space-separated list. Nothing else needs registering — the PR body's "Affected Apps" list is generated from the diff, so it cannot fall out of sync.
...
There is no second list to update. The PR body's "Affected Apps" section is built from what the update step actually changed, so a newly registered app appears there automatically.

**`APP_DIRS` remains the single point of failure.** An app missing from it is silently skipped by every base image update — no error, no warning, it just quietly falls behind. Auto-discovering `*/config.yaml` the way `builder.yml` does would remove that class of bug; until then, adding the slug is a required manual step.
...
- **2026-03-14:** `huly` was missing from the update pipeline and was registered.
- **2026-07-31 (#196):** the PR body's "Affected Apps" list was a hand-maintained literal naming 6 apps while the updater changed 11. It is now derived from the diff. Verified by rolling every `build.yaml` back and running the real updater: 11 apps, matching the repo.

All apps carrying a `build.yaml` are tracked. Get the current count from the repo rather than from this file — `ls -d */config.yaml | wc -l` — so this note cannot go stale the way the list it replaced did.
...
| `.github/workflows/update-base-image.yml` | *nothing* — "Affected Apps" is derived from the diff | — |
