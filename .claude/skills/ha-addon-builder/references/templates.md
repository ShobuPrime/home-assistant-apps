...
profile <app_slug> flags=(attach_disconnected,mediate_deleted) {
  ...
  capability,
  file,
  # Both directions. AppArmor mediates signal DELIVERY as well as sending, so
  # a send-only rule (or none at all) stops s6's SIGTERM ever reaching the app:
  # graceful shutdown is skipped and the container is SIGKILLed (exit 137).
  # .github/scripts/validate-apparmor.sh rule 4 fails the PR on anything that
  # does not grant `receive`, so a send-only profile never reaches master.
  signal (send,receive),
  network,
  mount,
...
- **If the app needs to execute child processes**: exec is granted by the bare `file,` rule (measured: a bare `file,` does grant exec) together with the explicit `ix` path lists above — `capability,` has nothing to do with it, it grants Linux capabilities. Add an `ix` rule for any directory you execute from that the lists do not already cover (e.g. `/usr/sbin/** ix,` for nginx).
