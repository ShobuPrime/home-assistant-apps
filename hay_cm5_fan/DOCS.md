...
Entity IDs are `<domain>.hay_cm5_<thing>` — for example `sensor.hay_cm5_processor_temperature`
and `binary_sensor.hay_cm5_fan`.

Home Assistant composes these from the MQTT **device name** ("HAY CM5") and the **entity name**
("Processor Temperature"); the discovery payload's `object_id` does not drive it, because these
entities carry a device and `has_entity_name`. That is why the entity names deliberately omit a
"CM5 " prefix — including it produced the doubled
`sensor.hay_cm5_fan_controller_cm5_processor_temperature` that earlier versions created.

Because they come from MQTT discovery, every entity has a **unique ID**, so it can be renamed,
hidden, or assigned to an area from the Home Assistant UI. Entities created by the REST fallback
cannot have unique IDs — which is the other reason the fallback is now a brief, self-correcting
state rather than something an unlucky boot can latch you into for a day.

### Primary Entities

#### `sensor.hay_cm5_processor_temperature`
...
- `sensor.hay_cm5_nvme_temperature` — NVMe SSD temperature (if present)
- `sensor.hay_cm5_io_controller_temperature` — RP1 I/O controller temperature
...
hours_to_show: 24
entities:
  - entity: sensor.hay_cm5_processor_temperature
    name: CPU
  - entity: sensor.hay_cm5_nvme_temperature
    name: NVMe SSD
```
...
entities:
  - sensor.hay_cm5_processor_temperature
  - sensor.hay_cm5_nvme_temperature
```
...
card:
  ...
  content: >
    CPU is at {{ states('sensor.hay_cm5_processor_temperature') }}C.
    Fan is {{ states('binary_sensor.hay_cm5_fan') }}.
```
    ...
    action:
      - service: notify.mobile_app
        data:
          ...
          message: >
            CPU temperature is {{ states('sensor.hay_cm5_processor_temperature') }}C
            with fan {{ states('binary_sensor.hay_cm5_fan') }}.
...
automation:
  - alias: "HAY Fan State Changed"
    trigger:
      - platform: state
        entity_id: binary_sensor.hay_cm5_fan
    action:
      - service: logbook.log
        data:
          name: "CM5 Fan"
          message: "Fan turned {{ states('binary_sensor.hay_cm5_fan') }} at CPU {{ states('sensor.hay_cm5_processor_temperature') }}C"
```
